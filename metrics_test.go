...
func TestP8sServer(t *testing.T) {
	...
	var stdout bytes.Buffer
	StartReader("test_module", reader, &stdout, os.Stderr)
	...
	if !strings.Contains(body, "test_module_total_bytes_total") {
		t.Errorf("HTTP response:\n%s\n does not contain expected %s", body, "test_module_total_bytes_total")
	}
	if !strings.Contains(body, "total_request_count_total") {
		t.Errorf("HTTP response:\n%s\n does not contain expected %s", body, "test_module_total_request_count_total")
	}
