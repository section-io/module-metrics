...
func TestSetupModule(t *testing.T) {
	...
	err := SetupModule(fifoFilePath, &stdout, os.Stderr, []string{})
	if err != nil {
		t.Error(err)
	}
...
func TestSetupModulePassesAdditionalLabels(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping test in short mode.")
	}

	var stdout bytes.Buffer
	err := SetupModule(fifoFilePath, &stdout, os.Stderr, []string{"http_x_forwarded_proto"})
	if err != nil {
		t.Error(err)
	}
	...
	expected := `section_http_request_count_total{hostname="bar.example.com",http_x_forwarded_proto="https",status="304"} 1`
	if !strings.Contains(actual, expected) {
		t.Errorf("Output:\n%s\n does not contain expected %s", actual, expected)
	}

	expected = `section_http_bytes_total{hostname="www.example.com",http_x_forwarded_proto="https",status="304"} 1790`
	if !strings.Contains(actual, expected) {
		t.Errorf("Output:\n%s\n does not contain expected %s", actual, expected)
	}
}
